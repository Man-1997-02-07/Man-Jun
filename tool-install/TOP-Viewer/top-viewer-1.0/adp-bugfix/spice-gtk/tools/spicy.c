...
static gboolean watermark_updater(gpointer data) 
{
    SpiceWindow *win = data;
	gfloat xalign, yalign;
 
	xalign = gtk_label_get_xalign(GTK_LABEL(win->label));
	yalign = gtk_label_get_yalign(GTK_LABEL(win->label));
...
static void new_file_transfer_window(GtkWidget *widget, SpiceFileTransferTask *task, gpointer user_data);
                                        ...
                                        gpointer *user_data)
{
    ...
    spice_main_channel_file_copy_async(self->priv->main, files, 0, NULL, NULL, NULL, file_transfer_cb,
                                       NULL);
    for (i = 0; i < n_files; i++) {
...
static SpiceWindow *create_spice_window(spice_connection *conn, SpiceChannel *channel, int id, gint monitor_id)
{
	...
	gtk_window_set_default_size (GTK_WINDOW (win->toplevel), 1440, 1080);
	...
	gtk_drag_dest_set(GTK_WIDGET(win->spice), GTK_DEST_DEFAULT_ALL, &targets, 1, GDK_ACTION_COPY);
	g_signal_connect(G_OBJECT(win->spice), "drag-data-received",
                     G_CALLBACK(drag_data_received_cb), SPICE_DISPLAY(win->spice));	
	...
	gtk_overlay_add_overlay (GTK_OVERLAY (win->overlay), vbox); 
	gtk_overlay_set_overlay_pass_through (GTK_OVERLAY (win->overlay), vbox, TRUE);
	
	win->label = gtk_label_new ("<span foreground='black' weight='20' font='9'>张朝潞8070\n13560736814</span>");
	gtk_label_set_use_markup (GTK_LABEL (win->label), TRUE);

    gtk_overlay_add_overlay (GTK_OVERLAY (win->overlay), win->label); 
	gtk_overlay_set_overlay_pass_through (GTK_OVERLAY (win->overlay), win->label, TRUE);
	...
	gtk_overlay_reorder_overlay(GTK_OVERLAY (win->overlay), GTK_WIDGET(vbox), -1); 

    
    g_timeout_add_seconds (3, watermark_updater, win);
...
static void new_file_transfer(SpiceMainChannel *main, SpiceFileTransferTask *task, gpointer user_data)
{
    g_print("new_file_transfer entry...\n");
    spice_connection *conn = user_data;
