...
static gchar *read_usbredir_set_file(void)
{
    GKeyFile *topsec_keyfile = NULL;
    gchar *topsec_conf_file, *vid, *pid;
    GError *error = NULL;
    gsize groups = 0;
    gchar *redir_filter = g_new0(gchar, 512);
    ...
    g_key_file_get_groups(topsec_keyfile, &groups);
    if (0 != groups) {
        for(int i = 1; i<=groups; ++i){
            if (g_key_file_get_boolean(topsec_keyfile, g_strdup_printf("%d", i), "switch", &error)) {
                vid = g_key_file_get_string(topsec_keyfile, g_strdup_printf("%d", i), "vid", &error);
                pid = g_key_file_get_string(topsec_keyfile, g_strdup_printf("%d", i), "pid", &error);
                if (groups > 1 && i != groups) {
                    redir_filter = g_strdup_printf("%s%s%s%s%s%s", redir_filter, "-1,0x", vid, ",0x", pid, ",-1,1|");
                }else if(i == groups || 1 == groups){
                    redir_filter = g_strdup_printf("%s%s%s%s%s%s", redir_filter, "-1,0x", vid, ",0x", pid, ",-1,1");
                }
            }
        }
...
void spice_set_session_option(SpiceSession *session)
{
    ...
    if (1) {
        ...
        if (m && usbredir_redirect_on_connect) {
            g_object_set(m, "redirect-on-connect",
                         usbredir_redirect_on_connect, NULL);
        }else if(m && !usbredir_redirect_on_connect){
            gchar *topsce_usbredir_redirect_on_connect = read_usbredir_set_file();
            if (topsce_usbredir_redirect_on_connect) {
                g_message("topsce_usbredir_redirect_on_connect : %s", topsce_usbredir_redirect_on_connect);
                g_object_set(m, "redirect-on-connect",
                             topsce_usbredir_redirect_on_connect, NULL);
            }
