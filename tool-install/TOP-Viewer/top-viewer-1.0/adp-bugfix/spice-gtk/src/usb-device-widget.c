...
static void spice_usb_device_widget_constructed(GObject *gobject)
{
    ...
    //devices = spice_usb_device_manager_get_devices_with_filter(priv->manager, "0x03,-1,-1,-1,0|-1,-1,-1,-1,1");
...
static void write_usbredir_set_file(SpiceUsbDevice *device, gboolean click, int all_channels);
...
static void write_usbredir_set_file(SpiceUsbDevice *device, gboolean click, int all_channels){
     gint fd;
     GKeyFile *topsec_keyfile = NULL;
     GError *error = NULL;
     gchar *topsec_conf_file, *topsec_conf;
     gint max_group = all_channels;
     topsec_keyfile = g_key_file_new();
     topsec_conf_file = g_build_filename(g_get_user_config_dir(), "TopUsbRedir", NULL);
     if (g_mkdir_with_parents(topsec_conf_file, S_IRWXU) == -1){
         g_debug("topsec mkdir error");
     }
     g_free(topsec_conf_file);
     topsec_conf_file = g_build_filename(g_get_user_config_dir(), "TopUsbRedir", "settings", NULL);
     fd = g_open(topsec_conf_file,O_CREAT);
     g_close(fd, &error);
     if (error != NULL) {
         g_warning("g_close TopUsbRedir->settings error : %s", error->message);
     }
     if (!g_key_file_load_from_file(topsec_keyfile, topsec_conf_file,
                                   G_KEY_FILE_KEEP_COMMENTS|G_KEY_FILE_KEEP_TRANSLATIONS, &error)) {
         g_debug("zkm:Couldn't load configuration: %s", error->message);
         g_clear_error(&error);    
     }
     while (!g_key_file_has_group(topsec_keyfile, g_strdup_printf("%d", max_group))) {
         if (0 == max_group) {
             break;
         }
         --max_group;
     }
     g_key_file_set_boolean(topsec_keyfile, g_strdup_printf("%d", max_group%all_channels+1), "switch", click ? 1 : 0);
     g_key_file_set_string(topsec_keyfile, g_strdup_printf("%d", max_group%all_channels+1), "vid", spice_usb_device_get_description(device, "vid"));
     g_key_file_set_string(topsec_keyfile, g_strdup_printf("%d", max_group%all_channels+1), "pid", spice_usb_device_get_description(device, "pid"));
     if ((topsec_conf = g_key_file_to_data(topsec_keyfile, NULL, &error)) == NULL ||
        !g_file_set_contents(topsec_conf_file, topsec_conf, -1, &error)){
         g_debug("zkm:save error"); 
     }
     g_free(topsec_conf_file);
     g_free(topsec_conf);
     g_key_file_free(topsec_keyfile);
}
...
static void checkbox_clicked_cb(GtkWidget *check, gpointer user_data)
{
    ...
    if (NULL != priv->manager) {
        g_object_get(priv->manager, "all-channels", &all_channels, NULL);
    }

    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(check))) {
        write_usbredir_set_file(device, 1, all_channels);
        ...
        write_usbredir_set_file(device, 0, all_channels);
                            ...
                            gpointer user_data)
{
    ...
    desc = spice_usb_device_get_description(device,
                                            priv->device_format_string);
    //g_debug("ZKM MARK: device description is %s", desc);
    if (NULL != g_strrstr(desc, "mouse") || NULL != g_strrstr(desc, "Mouse") || NULL != g_strrstr(desc, "keyboard") || NULL != g_strrstr(desc, "Keyboard")) {
        return;
    }
