                                    ...
                                    GParamSpec *pspec)
{
    ...
    case PROP_MAX_CLIPBOARD:
        ...
        break;
    case PROP_DISABLE_XFER_FILE:
	g_value_set_boolean(value, c->disable_xfer_file);
	break;
    default:
                                    ...
                                    const GValue *value, GParamSpec *pspec)
{
    ...
    case PROP_MAX_CLIPBOARD:
        ...
        break;
    case PROP_DISABLE_XFER_FILE:
	c->disable_xfer_file = g_value_get_boolean(value);
	break;
    default:
...
static void spice_main_channel_class_init(SpiceMainChannelClass *klass)
{
    ...
    g_object_class_install_property
        (gobject_class, PROP_DISABLE_XFER_FILE,
         g_param_spec_boolean("disable-xfer-file",
                              "Disable xfer file",
                              "Disable drag file  into guest",
                              FALSE,
                              G_PARAM_READWRITE |
                              G_PARAM_STATIC_STRINGS));
...
void change_spice_main_channel_drag_file(SpiceMainChannel *channel, gboolean enabled){
        g_object_set(channel, "disable-xfer-file", !enabled, NULL);
}
                                        ...
                                        gpointer user_data)
{
    ...
    GError *error = NULL;
    GList *it, *keys;
    gboolean disable_xfer_file = FALSE;

    g_return_if_fail(channel != NULL);

    //change_spice_main_channel_drag_file(channel, FALSE);
    g_object_get(channel, "disable-xfer-file", &disable_xfer_file, NULL);

    if (disable_xfer_file) {
        SPICE_DEBUG("Disable drag file to guest");
    }else{
        SPICE_DEBUG("Able drag file to guest");
    }

    g_return_if_fail(disable_xfer_file != 1);
    g_return_if_fail(SPICE_IS_MAIN_CHANNEL(channel));
