                                                  ...
                                                  GParamSpec  *pspec)
{
    ...
    case PROP_ALL_CHANNELS: {
        int all_channels = 0;
#ifdef USE_USBREDIR
        all_channels = priv->channels->len;
#endif
        g_value_set_int(value, all_channels);
        break;
    }
    default:
...
static void spice_usb_device_manager_class_init(SpiceUsbDeviceManagerClass *klass)
{
    ...
    pspec = g_param_spec_int("all-channels", "all channels",
                             "The number of all channels for redirecting USB devices",
                             0,
                             G_MAXINT,
                             0,
                             G_PARAM_READABLE);
    g_object_class_install_property(gobject_class, PROP_ALL_CHANNELS, pspec);
...
gchar *spice_usb_device_get_description(SpiceUsbDevice *device, const gchar *format)
{
    ...
    if (g_str_equal(format, "vid")) {
        return g_strdup_printf("%04x", spice_usb_device_get_vid(device));
    }
    if (g_str_equal(format, "pid")) {
        return g_strdup_printf("%04x", spice_usb_device_get_pid(device));
    }
