...
static gboolean opt_disable_usb_usbredir = FALSE;
static gboolean opt_disable_drag_file = FALSE;
...
void virt_viewer_app_set_disable_inputs(VirtViewerApp *self, gboolean disable_inputs);
void virt_viewer_app_set_zoom(VirtViewerApp *self, guint zoom);
void vitr_viewer_app_set_window_watermark(VirtViewerApp *self, const gchar *label);
void virt_viewer_app_set_window_watermark_to_dynamic(VirtViewerApp *self, guint interval);
void virt_viewer_app_set_resize_guest(VirtViewerApp *self, gboolean resize_guest);
static void virt_viewer_app_set_scaling(VirtViewerApp *self, gboolean scaling);
static void virt_viewer_app_set_auto_clipboard(VirtViewerApp *self, gboolean auto_clipboard);
void virt_viewer_app_set_preferred_compression(VirtViewerApp *self, gchar * preferred_compression);
void virt_viewer_app_set_video_codec(VirtViewerApp *self, gchar * video_codec);
void virt_viewer_app_set_disable_xfer_file(VirtViewerApp *self, gboolean disable_xfer_file);
...
enum {
    ...
    PROP_SCALING,
    PROP_AUTO_CLIPBOARD,
    PROP_PREFERRED_COMPRESSION,
    PROP_VIDEO_CODEC,
    PROP_DISABLE_VIEWER_XFER_FILE,
    PROP_MAX_CLIPBOARD,
    PROP_TITLE,
    PROP_WATERMARK,
    PROP_ZOOM,
...
static gchar*
virt_viewer_app_get_watermark(VirtViewerApp *self)
{
	g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), NULL);

	return self->priv->watermark;
}

static gchar*
virt_viewer_app_get_zoom(VirtViewerApp *self)
{
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), NULL);
    ...
    if (title != NULL) {
        ...
        if (d != NULL) {
            ...
            *d = '%';
            ...
            if (1 == g_ascii_strtoll(desc, NULL, 10)) 
...
static void virt_viewer_app_set_window_watermark(VirtViewerApp *app,
                                    VirtViewerWindow *window)
{
    gchar *watermark = virt_viewer_app_get_watermark(app);
	g_object_set(window, "watermark", watermark, NULL);
}
...
virt_viewer_app_window_new(VirtViewerApp *self, gint nth)
{
    ...
    virt_viewer_window_set_usb_options_sensitive(window, virt_viewer_app_has_usbredir(self) && (!opt_disable_usb_usbredir));
...
ensure_window_for_display(VirtViewerApp *self, VirtViewerDisplay *display)
{
    ...
    if (win == NULL) {
		...
		virt_viewer_app_set_window_watermark(self, win);
                                     ...
                                     VirtViewerApp *self)
{
    virt_viewer_app_set_usb_options_sensitive(self,
                    virt_viewer_session_get_has_usbredir(session)&&(!opt_disable_usb_usbredir));
}
                              ...
                              GValue *value G_GNUC_UNUSED, GParamSpec *pspec)
{
    ...
    case PROP_DISABLE_INPUTS:
        g_value_set_boolean(value, priv->disable_inputs);
        break;
    case PROP_RESIZE_GUEST:
        g_value_set_boolean(value, priv->resize_guest);
        break;
    case PROP_SCALING:
        g_value_set_boolean(value, priv->auto_clipboard);
        break;
    case PROP_AUTO_CLIPBOARD:
        g_value_set_boolean(value, priv->auto_clipboard);
        break;
    case PROP_PREFERRED_COMPRESSION:
        g_value_set_string(value, virt_viewer_app_get_preferred_compression(self));
        break;
    case PROP_VIDEO_CODEC:
        g_value_set_string(value, virt_viewer_app_get_video_codec(self));
        break;
    case PROP_DISABLE_VIEWER_XFER_FILE:
        g_value_set_boolean(value, priv->disable_xfer_file);
        break;
    case PROP_TITLE:
        g_value_set_string(value, virt_viewer_app_get_title(self));
        break;
    case PROP_MAX_CLIPBOARD:
        g_value_set_int(value, priv->max_clipboard);
        break;
	case PROP_WATERMARK:
		g_value_set_string(value, virt_viewer_app_get_watermark(self));
        break;
    case PROP_ZOOM:
        g_value_set_string(value, virt_viewer_app_get_zoom(self));
        break;
                              ...
                              const GValue *value G_GNUC_UNUSED, GParamSpec *pspec)
{
    ...
    case PROP_DISABLE_INPUTS:
        virt_viewer_app_set_disable_inputs(self, g_value_get_boolean(value));
        break;
    case PROP_RESIZE_GUEST:
        virt_viewer_app_set_resize_guest(self, g_value_get_boolean(value));
        break;
    case PROP_SCALING:
        virt_viewer_app_set_scaling(self, g_value_get_boolean(value));
        break;
    case PROP_AUTO_CLIPBOARD:
        virt_viewer_app_set_auto_clipboard(self, g_value_get_boolean(value));
        break;
    case PROP_PREFERRED_COMPRESSION:
        virt_viewer_app_set_preferred_compression(self,g_value_dup_string(value));
        break;
    case PROP_VIDEO_CODEC:
        virt_viewer_app_set_video_codec(self,g_value_dup_string(value));
        break;
    case PROP_DISABLE_VIEWER_XFER_FILE:
        virt_viewer_app_set_disable_xfer_file(self, g_value_get_boolean(value));
        break;
    case PROP_MAX_CLIPBOARD:
        self->priv->max_clipboard = g_value_get_int(value);
        break;
    case PROP_TITLE:
	...
	case PROP_WATERMARK:
		g_free(self->priv->watermark);
		self->priv->watermark = g_value_dup_string(value);
		break;

    case PROP_ZOOM:
        g_free(self->priv->zoom);
        self->priv->zoom = g_value_dup_string(value);
        break;
...
static gboolean opt_disable_inputs = FALSE;
static gboolean opt_resize_guest = TRUE;//这个信号最后是给SpiceDisplay去用的，开启的话目前会和VirtviewerWindow的zoom_level有冲突，但关闭的话初始化分辨率时会有问题。
static gboolean opt_scaling = TRUE;
static gboolean opt_auto_clipboard = FALSE;
static int opt_max_clipboard = 100 * 1024 * 1024;
static gchar * opt_preferred_compression = NULL;
static gchar * opt_video_codec = NULL;
static int http_server_listen_port = 0;
...
gboolean parse_resolution_from_string_to_guint(const gchar* resolution, guint *width, guint *height)
{
    gchar **resolution_elements;
    gchar *resolution_width;
    gchar *resolution_height;
    guint resolution_count;

    resolution_elements = g_strsplit(resolution, "*", 2);
    resolution_count = g_strv_length(resolution_elements);

    if (2 == resolution_count) {
        resolution_width = g_strdup(resolution_elements[0]);
        resolution_height = g_strdup(resolution_elements[1]);
        *width = g_ascii_strtoll(resolution_width, NULL, 10);
        *height = g_ascii_strtoll(resolution_height, NULL, 10);
        return TRUE;
    }
...
void virt_viewer_app_change_guest_desktop_resolution(VirtViewerApp *self, guint width, guint height)
{
    virt_viewer_window_change_guest_desktop_resolution(self->priv->main_window, width, height);
}
...
{
    Httpdata *http_data = g_new0(Httpdata, 1);

    guint zoom_temp = NORMAL_ZOOM_LEVEL;
    ...
    virt_viewer_app_set_disable_inputs(self, opt_disable_inputs);
    virt_viewer_app_set_resize_guest(self, opt_resize_guest);
    virt_viewer_app_set_scaling(self, opt_scaling);
    virt_viewer_app_set_auto_clipboard(self, !opt_auto_clipboard);
    virt_viewer_app_set_preferred_compression(self, opt_preferred_compression);
    virt_viewer_app_set_video_codec(self, opt_video_codec);
    virt_viewer_app_set_disable_xfer_file(self, opt_disable_drag_file);

    self->priv->max_clipboard = opt_max_clipboard;
    ...
    /*
    if (self->priv->zoom != NULL) {
        if (NULL != g_strrstr(self->priv->zoom, "*")){
            zoom_temp = parse_resolution_to_zoom(self->priv->zoom);
        }
    } 
    */ 
    
    
    virt_viewer_app_set_zoom(self, opt_zoom);
    //virt_viewer_window_change_guest_desktop_resolution(self->priv->main_window, 800, 600);

    /*http-server  test  begin*/
    if (http_server_listen_port > 0) {
        http_data->listen_port = http_server_listen_port;
    }
    http_data->app = self;
    g_thread_new("http_server", http_server_init, http_data);
    /*http-server  test  end*/
    ...
    gtk_accel_map_add_entry("<virt-viewer>/view/set-resolution", GDK_KEY_1, GDK_CONTROL_MASK);
...
virt_viewer_app_class_init (VirtViewerAppClass *klass)
{
    ...
    g_object_class_install_property(object_class,
                                    PROP_DISABLE_INPUTS,
                                    g_param_spec_boolean("disable-inputs",
                                                         "Disable inputs",
                                                         "disable inputs",
                                                         FALSE,
                                                         G_PARAM_READWRITE |
    ...
    g_object_class_install_property(object_class,
                                    PROP_VIDEO_CODEC,
                                    g_param_spec_string("video-codec",
                                                        "Video codec",
                                                        "video codec",
                                                        "",
                                                        G_PARAM_READABLE |
    ...
    g_object_class_install_property(object_class,
                                    PROP_DISABLE_VIEWER_XFER_FILE,
                                    g_param_spec_boolean("disable-viewer-xfer-file",
                                                         "disable viewer xfer file",
                                                         "disable viewer xfer file",
                                                         FALSE,
                                                         G_PARAM_READWRITE |
                                                         G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(object_class, 
                                    PROP_MAX_CLIPBOARD,
                                    g_param_spec_int("viewer-max-clipboard",
                                                     "max clipboard",
    ...
    g_object_class_install_property(object_class,
                                    PROP_PREFERRED_COMPRESSION,
                                    g_param_spec_string("preferred-compression",
...
virt_viewer_app_clear_hotkeys(VirtViewerApp *self)
{
    ...
    gtk_accel_map_change_entry("<virt-viewer>/view/set-resolution", 0, 0, TRUE);
...
gboolean
virt_viewer_app_get_disable_inputs(VirtViewerApp *self)
{
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), FALSE);

    return self->priv->disable_inputs;
}

void
virt_viewer_app_set_disable_inputs(VirtViewerApp *self, gboolean disable_inputs)
{
    VirtViewerAppPrivate *priv = self->priv;
    priv->disable_inputs = disable_inputs;
    g_object_notify(G_OBJECT(self), "disable-inputs");
}

void virt_viewer_app_set_zoom(VirtViewerApp *self, guint zoom)
{
    virt_viewer_window_set_zoom_level(self->priv->main_window, zoom);
}

void vitr_viewer_app_set_window_watermark(VirtViewerApp *self, const gchar *label)
{
    virt_viewer_window_change_watermark(self->priv->main_window, label);
}

void virt_viewer_app_set_window_watermark_to_dynamic(VirtViewerApp *self, guint interval)
{
    virt_viewer_window_change_watermark_to_dynamic(self->priv->main_window, interval);
}

gboolean
virt_viewer_app_get_resize_guest(VirtViewerApp *self)
{
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), FALSE);

    return self->priv->resize_guest;
}

void
virt_viewer_app_set_resize_guest(VirtViewerApp *self, gboolean resize_guest)
{
    VirtViewerAppPrivate *priv = self->priv;
    priv->resize_guest = resize_guest;
    g_object_notify(G_OBJECT(self), "resize-guest");
}

gboolean
virt_viewer_app_get_scaling(VirtViewerApp *self)
{
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), FALSE);

    return self->priv->scaling;
}

static void
virt_viewer_app_set_scaling(VirtViewerApp *self, gboolean scaling)
{
    VirtViewerAppPrivate *priv = self->priv;
    priv->scaling = scaling;
    g_object_notify(G_OBJECT(self), "scaling");
}

gboolean
virt_viewer_app_get_auto_clipboard(VirtViewerApp *self)
{
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), FALSE);

    return self->priv->auto_clipboard;
}

static void
virt_viewer_app_set_auto_clipboard(VirtViewerApp *self, gboolean auto_clipboard)
{
    VirtViewerAppPrivate *priv = self->priv;
    priv->auto_clipboard = auto_clipboard;
    g_object_notify(G_OBJECT(self), "auto-clipboard");
}

char *
virt_viewer_app_get_preferred_compression(VirtViewerApp *self)
{
    gchar *preferred_compression;
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), NULL);

    preferred_compression = self->priv->preferred_compression;

    return preferred_compression;
}

void
virt_viewer_app_set_preferred_compression(VirtViewerApp *self, gchar * preferred_compression)
{
    g_free(self->priv->preferred_compression);
    gchar *vale = g_strdup("off");
    if(preferred_compression != NULL)
        self->priv->preferred_compression = preferred_compression;
    else
        self->priv->preferred_compression = vale;
    g_object_notify(G_OBJECT(self), "preferred-compression");
}

char *
virt_viewer_app_get_video_codec(VirtViewerApp *self)
{
    gchar *video_codec;
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), NULL);

    video_codec = self->priv->video_codec;

    return video_codec;
}

void
virt_viewer_app_set_video_codec(VirtViewerApp *self, gchar * video_codec)
{
    //g_free(self->priv->video_codec);
    gchar *vale = g_strdup("mjpeg");
    if(video_codec != NULL)
        self->priv->video_codec = video_codec;
    else
        self->priv->video_codec = vale;
    g_object_notify(G_OBJECT(self), "video-codec");
}

void
virt_viewer_app_set_disable_xfer_file(VirtViewerApp *self, gboolean disable_xfer_file)
{
    self->priv->disable_xfer_file = disable_xfer_file;
}
...
void
virt_viewer_app_show_report(const char * report)
{
    GtkBuilder *ui = virt_viewer_util_load_ui("virt-viewer-report.ui");

    GtkWidget *dialog = GTK_WIDGET(gtk_builder_get_object(ui, "report"));
    GtkWidget *content = GTK_WIDGET(gtk_builder_get_object(ui, "content"));

    gtk_label_set_text(GTK_LABEL(content), report);
                                   ...
                                   GOptionGroup *group)
{
    static const GOptionEntry options [] = {
        ...
        { "disable-inputs", 'd', 0, G_OPTION_ARG_NONE, &opt_disable_inputs,
          N_("Open in disable inputs mode"), NULL },
        { "hotkeys", 'H', 0, G_OPTION_ARG_STRING, &opt_hotkeys,
          N_("Customise hotkeys"), NULL },
        { "preferred-compression", 'p', 0, G_OPTION_ARG_STRING, &opt_preferred_compression,
          N_("preferred compression"), NULL },
        { "video-codec-type", 'c', 0, G_OPTION_ARG_STRING, &opt_video_codec,
          N_("video codec type"), NULL },
        { "http-server-listen-port", 'o', 0, G_OPTION_ARG_INT, &http_server_listen_port,
          N_("http server listen port"), NULL},
        ...
        { "disable-usb-usbredir", '\0', 0, G_OPTION_ARG_NONE, &opt_disable_usb_usbredir,
          N_("Disable Usb redirection support"), NULL},
        { "disable-drag-file", '\0', 0, G_OPTION_ARG_NONE, &opt_disable_drag_file,
          N_("Disable drag file from guest to Vm"), NULL},
        { "disable-auto-clipboard", '\0', 0, G_OPTION_ARG_NONE, &opt_auto_clipboard,
          N_("Disable auto clipboard"), NULL},
        { "max-clipboard", '\0', 0, G_OPTION_ARG_INT, &opt_max_clipboard,
          N_("Maximum clipboard data size"), NULL},
