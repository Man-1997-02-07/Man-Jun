...
static void
auto_clipboard_changed(VirtViewerApp *app,
                       GParamSpec *pspec G_GNUC_UNUSED,
                       VirtViewerSessionSpice *self)
{
    gboolean auto_clipboard = virt_viewer_app_get_auto_clipboard(app);
    if (auto_clipboard) {
        g_debug("auto_clipboard is TRUE !!!");
    }
    else{
        g_debug("auto_clipboard is FALSE !!!");
    }
    g_object_set(self->priv->gtk_session, "auto-clipboard", auto_clipboard, NULL);
}
...
create_spice_session(VirtViewerSessionSpice *self)
{
    ...
    virt_viewer_signal_connect_object(app, "notify::auto-clipboard",
                                      G_CALLBACK(auto_clipboard_changed), self, 0);
    auto_clipboard_changed(app,NULL,self);
                                   ...
                                   const gchar *uri, GError **error)
{
	...
	g_object_get(self->priv->session, "username", &username, NULL);
    g_object_get(self->priv->session, "password", &password, NULL);
    ...
    g_object_get(self->priv->session, "uri", &uri, NULL);
    g_warning("open uri vvfile: %p username: %s, password: %s, uri: %s", file, username, password, uri);
                                               ...
                                               GtkWindow *parent)
{
    ...
    gtk_window_set_decorated(GTK_WINDOW(dialog), FALSE);//设置成无窗口边框模式
...
virt_viewer_session_spice_display_monitors(SpiceChannel *channel,
                                           ...
                                           VirtViewerSessionSpice *self)
{
    gchar *app_commandline_resolution = NULL;
    ...
    for (i = 0; i < monitors->len; i++) {
        ...
        video_codec_changed(virt_viewer_session_get_app(VIRT_VIEWER_SESSION(self)), NULL, VIRT_VIEWER_DISPLAY_SPICE(display));
        ...
        g_object_get(virt_viewer_session_get_app(VIRT_VIEWER_SESSION(self)), "zoom", &app_commandline_resolution, NULL);
        if (NULL != app_commandline_resolution) {
            guint resolution_width;
            guint resolution_height;
            if(parse_resolution_from_string_to_guint(app_commandline_resolution, &resolution_width, &resolution_height)){
                virt_viewer_display_spice_set_desktop(VIRT_VIEWER_DISPLAY(display), 0, 0, resolution_width, resolution_height);
            }else{
                virt_viewer_display_spice_set_desktop(VIRT_VIEWER_DISPLAY(display),
                                                      monitor->x, monitor->y,
                                                      monitor->width, monitor->height);
            }
            g_object_set(virt_viewer_session_get_app(VIRT_VIEWER_SESSION(self)), "zoom", NULL, NULL);
        }else{
            virt_viewer_display_spice_set_desktop(VIRT_VIEWER_DISPLAY(display),
                                                      monitor->x, monitor->y,
                                                      monitor->width, monitor->height);
            g_debug("Hit virt_viewer_session_spice_display_monitors: monitor->x=%u, monitor->y=%u, monitor->width=%u, monitor->height=%u\n",
                                      ...
                                      VirtViewerSession *session)
{
    ...
    VirtViewerSessionSpice *self = VIRT_VIEWER_SESSION_SPICE(session);
    int id, type;
    gboolean disable_viewer_xfer_file = FALSE;
    gint max_clipboard;

    g_object_get(app, "disable-viewer-xfer-file", &disable_viewer_xfer_file, NULL);
    g_object_get(app, "viewer-max-clipboard", &max_clipboard, NULL);
    if (disable_viewer_xfer_file) {
        g_debug("disable_viewer_xfer_file  =  1 !\n");
    }else{
        g_debug("disable_viewer_xfer_file  =  0 !\n");
    }

    g_debug("max_clipboard = %d", max_clipboard);
    ...
    if (SPICE_IS_MAIN_CHANNEL(channel)) {
        ...
        g_object_set(G_OBJECT(channel),
                     ...
                     "disable-xfer-file", disable_viewer_xfer_file,
                     "max-clipboard", max_clipboard,
                     NULL);
