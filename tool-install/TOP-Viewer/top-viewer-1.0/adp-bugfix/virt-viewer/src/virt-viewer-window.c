...
static GtkMenu* virt_viewer_window_get_preference_menu(VirtViewerWindow *self);
static GtkMenu* virt_viewer_window_get_file_menu(VirtViewerWindow *self);
...
struct _VirtViewerWindowPrivate {
	...
	gboolean overlay_lock;	//防止改变位置定时器函数重�?
	gchar *watermark_info; //水印内容，可以是HTML标签修饰的字符串
    gint zoomlevel;
    gboolean fullscreen;
    gchar *subtitle;
                                 ...
                                 GValue *value, GParamSpec *pspec)
{
	...
	case PROP_WATERMARK:
		g_value_set_string(value, priv->watermark_info);
		break;		
                                 ...
                                 const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_WATERMARK:
		g_free(priv->watermark_info);
		priv->watermark_info = g_value_dup_string(value);
	    g_print("PROP_WATERMARK: %s\n", priv->watermark_info);
	    break;
                   ...
                   gpointer    user_data)
{
    ...
    /*
    menu = GTK_WIDGET(gtk_builder_get_object(self->priv->builder, "menu-preference"));
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(menu),
                              GTK_WIDGET(virt_viewer_window_get_preference_menu(self)));
    */
...
virt_viewer_window_init (VirtViewerWindow *self)
{
    ...
    g_signal_connect(gtk_builder_get_object(priv->builder, "menu-view-zoom-out"),
                     "can-activate-accel", G_CALLBACK(can_activate_cb), self);
    g_signal_connect(gtk_builder_get_object(priv->builder, "menu-view-set-resolution"),
                     "can-activate-accel", G_CALLBACK(can_activate_cb), self);
...
virt_viewer_window_menu_view_zoom_out(GtkWidget *menu G_GNUC_UNUSED,
                                      VirtViewerWindow *self)
{
    g_object_set(self->priv->app, "resize-guest", FALSE, NULL);
...
void virt_viewer_window_change_guest_desktop_resolution(VirtViewerWindow *self, guint width, guint height)
{
    virt_viewer_display_spice_change_resolution(self->priv->display, width, height);
}


G_MODULE_EXPORT void
virt_viewer_window_menu_view_set_resolution(GtkWidget *menu G_GNUC_UNUSED,
                                            VirtViewerWindow *self)
{
    g_debug("call set resolution");
    ...
    dialog = gtk_dialog_new_with_buttons(_("Resize guest to"),
                                         GTK_WINDOW(window),
    ...
    gtk_window_set_decorated(GTK_WINDOW(dialog), FALSE);//���ó��޴��ڱ߿�ģʽ
    gtk_window_set_default_size(dialog, 200, 200);//�Զ������õ�ǰ���ڴ�С

    spin_width = gtk_spin_button_new_with_range(0, G_MAXINT, 10);
    spin_height = gtk_spin_button_new_with_range(0, G_MAXINT, 10);

  
    virt_viewer_display_get_desktop_size(self->priv->display, &width, &height);//拿的是虚拟机桌面的分辨率
    ...
    gtk_grid_attach(grid, gtk_label_new(_("Resize the guest display")), 0, 0, 2, 1);
    gtk_grid_attach(grid, gtk_label_new(_("width:")), 0, 2, 1, 1);
    gtk_grid_attach(grid, spin_width, 1, 2, 1, 1);
    gtk_grid_attach(grid, gtk_label_new(_("height:")), 0, 3, 1, 1);
    ...
    if (gtk_dialog_run(GTK_DIALOG (dialog)) == GTK_RESPONSE_APPLY){
        ...
        guint custom_width, custom_height;
        custom_width = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spin_width));
        custom_height = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spin_height));
        
        virt_viewer_display_spice_change_resolution(self->priv->display, custom_width, custom_height);//需要虚拟机内有spice-agent
...
virt_viewer_window_queue_resize(VirtViewerWindow *self)
{
    ...
    gtk_widget_get_preferred_size(child, &min_nat, &nat);
    g_debug("VirtViewerWindow's min_size = %d*%d", min_nat.width, min_nat.height);
    g_debug("guest resolution = %d*%d", width, height);
    g_debug("nat.width = %d, nat.height = %d", nat.width, nat.height);
    g_debug("border = %u", border);
    g_debug("VirtViewerWindow's zoom_level = %d\n", zoom_level);
    if (priv->watermark_info == NULL) {
        gtk_window_resize(GTK_WINDOW(priv->window), nat.width + border, nat.height + border);
    }else{
        gtk_window_resize(GTK_WINDOW(priv->window), (width*zoom_level)/100+border, (height*zoom_level)/100+border+25);
        //gtk_window_resize(GTK_WINDOW(priv->window), 1920, 1080);
    }
}

virt_viewer_window_change_watermark(VirtViewerWindow *self, const gchar* label)
{
    VirtViewerWindowPrivate *priv = self->priv;
    gtk_label_set_markup(GTK_LABEL (priv->label), label);
    //��������
    gtk_label_set_xalign(GTK_LABEL(priv->label), watermark_x);
    gtk_label_set_yalign(GTK_LABEL(priv->label), watermark_y);
}
...
static void
resizeGuest_cb(GtkWidget *menu G_GNUC_UNUSED,VirtViewerWindow *self)
{
    g_object_set(self->priv->app, "resize-guest", gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menu)), NULL);
}

static void
scaling_cb(GtkWidget *menu G_GNUC_UNUSED,VirtViewerWindow *self)
{
    g_object_set(self->priv->app, "scaling", gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menu)), NULL);
}


static void
disableInputs_cb(GtkWidget *menu G_GNUC_UNUSED,VirtViewerWindow *self)
{
    g_object_set(self->priv->app, "disable-inputs", gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menu)), NULL);
}

static void
autoClipboard_cb(GtkWidget *menu G_GNUC_UNUSED,VirtViewerWindow *self)
{
    g_object_set(self->priv->app, "auto-clipboard", gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menu)), NULL);
}


static void
imageCompression_cb(GtkWidget *menu G_GNUC_UNUSED,VirtViewerWindow *self)
{
    gchar * preferred_compression = virt_viewer_app_get_preferred_compression(self->priv->app);
    if(g_strcmp0(preferred_compression, gtk_menu_item_get_label(GTK_MENU_ITEM(menu)))&& gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menu)))
        g_object_set(self->priv->app, "preferred-compression", gtk_menu_item_get_label(GTK_MENU_ITEM(menu)), NULL);
}

static GtkMenu*
virt_viewer_window_get_imageCompression_menu(VirtViewerWindow *self){

    GtkWidget * list;
    GtkMenu * menu = GTK_MENU(gtk_menu_new());
    gchar * preferred_compression = virt_viewer_app_get_preferred_compression(self->priv->app);

    list = gtk_radio_menu_item_new_with_label(NULL, "auto-glz");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(preferred_compression, "auto-glz"));
    g_signal_connect(list,"toggled",G_CALLBACK(imageCompression_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"auto-lz");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(preferred_compression, "auto-lz"));
    g_signal_connect(list,"toggled",G_CALLBACK(imageCompression_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"quic");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(preferred_compression, "quic"));
    g_signal_connect(list,"toggled",G_CALLBACK(imageCompression_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"glz");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(preferred_compression, "glz"));
    g_signal_connect(list,"toggled",G_CALLBACK(imageCompression_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"lz");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(preferred_compression, "lz"));
    g_signal_connect(list,"toggled",G_CALLBACK(imageCompression_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"lz4");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(preferred_compression, "lz4"));
    g_signal_connect(list,"toggled",G_CALLBACK(imageCompression_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"off");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(preferred_compression, "off"));
    g_signal_connect(list,"toggled",G_CALLBACK(imageCompression_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);
...
static void
videoCodecType_cb(GtkWidget *menu G_GNUC_UNUSED,VirtViewerWindow *self)
{
    gchar * videoCodecType = virt_viewer_app_get_video_codec(self->priv->app);
    if(g_strcmp0(videoCodecType, gtk_menu_item_get_label(GTK_MENU_ITEM(menu)))&&gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menu)))
        g_object_set(self->priv->app, "video-codec", gtk_menu_item_get_label(GTK_MENU_ITEM(menu)), NULL);
}

static GtkMenu*
virt_viewer_window_get_videoCodecType_menu(VirtViewerWindow *self){
    GtkWidget * list;
    GtkMenu * menu = GTK_MENU(gtk_menu_new());
    gchar * videoCodecType = virt_viewer_app_get_video_codec(self->priv->app);

    list = gtk_radio_menu_item_new_with_label(NULL, "mjpeg");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(videoCodecType, "mjpeg"));
    g_signal_connect(list,"toggled",G_CALLBACK(videoCodecType_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"vp8");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(videoCodecType, "vp8"));
    g_signal_connect(list,"toggled",G_CALLBACK(videoCodecType_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);

    /*
    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"vp9");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(videoCodecType, "vp9"));
    g_signal_connect(list,"toggled",G_CALLBACK(videoCodecType_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);
    */
    list = gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(list)),"h264");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(list),!g_strcmp0(videoCodecType, "h264"));
    g_signal_connect(list,"toggled",G_CALLBACK(videoCodecType_cb),self);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), list);
...
static GtkMenu*
virt_viewer_window_get_preference_menu(VirtViewerWindow *self)
{
    GtkWidget * resizeGuestItem;
    GtkWidget * scalingItem;
    GtkWidget * disableInputsItem;
    GtkWidget * autoClipboardItem;
    GtkWidget * imageCompressionItem;
    GtkWidget * videoCodecTypeItem;
    ...
    resizeGuestItem = gtk_check_menu_item_new_with_label(_("resize guest"));
    scalingItem = gtk_check_menu_item_new_with_label(_("scaling"));
    disableInputsItem = gtk_check_menu_item_new_with_label(_("disable inputs"));
    autoClipboardItem = gtk_check_menu_item_new_with_label(_("auto clipboard"));
    imageCompressionItem = gtk_menu_item_new_with_label(_("image compression"));
    videoCodecTypeItem = gtk_menu_item_new_with_label(_("video codec type"));

    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(resizeGuestItem),virt_viewer_app_get_resize_guest(priv->app));
    g_signal_connect(G_OBJECT(resizeGuestItem),"toggled",G_CALLBACK(resizeGuest_cb),self);

    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(scalingItem),virt_viewer_app_get_scaling(priv->app));
    g_signal_connect(G_OBJECT(scalingItem),"toggled",G_CALLBACK(scaling_cb),self);

    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(disableInputsItem),virt_viewer_app_get_disable_inputs(priv->app));
    g_signal_connect(G_OBJECT(disableInputsItem),"toggled",G_CALLBACK(disableInputs_cb),self);

    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(autoClipboardItem),virt_viewer_app_get_auto_clipboard(priv->app));
    g_signal_connect(G_OBJECT(autoClipboardItem),"toggled",G_CALLBACK(autoClipboard_cb),self);


    gtk_menu_item_set_submenu(GTK_MENU_ITEM(imageCompressionItem), virt_viewer_window_get_imageCompression_menu(self) );
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(videoCodecTypeItem), virt_viewer_window_get_videoCodecType_menu(self) );



    gtk_menu_shell_append(GTK_MENU_SHELL(menu), resizeGuestItem );
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), scalingItem );
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), disableInputsItem );
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), autoClipboardItem );
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), imageCompressionItem );
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), videoCodecTypeItem );
...
static GtkMenu*
virt_viewer_window_get_file_menu(VirtViewerWindow *self)
{
    //GtkWidget * screenshot;
    GtkWidget * usb_device_selection;
    ...
    //screenshot = gtk_menu_item_new_with_label(_("screenshot"));
    usb_device_selection = gtk_menu_item_new_with_label(_("usb device selection"));
    //preferences = gtk_menu_item_new_with_label(_("folders sharing"));

    //g_signal_connect(GTK_MENU(screenshot),"activate",G_CALLBACK(virt_viewer_window_menu_file_screenshot),self);
    g_signal_connect(GTK_MENU_ITEM(usb_device_selection),"activate",G_CALLBACK(virt_viewer_window_menu_file_usb_device_selection),self);
    //g_signal_connect(GTK_MENU(preferences),"activate",G_CALLBACK(virt_viewer_window_menu_preferences_cb),self);

    //gtk_menu_shell_append(GTK_MENU_SHELL(menu), screenshot );
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), usb_device_selection );
...
static void
action_set_resolution(G_GNUC_UNUSED GSimpleAction *action,
                      G_GNUC_UNUSED GVariant *state,
                      gpointer user_data)
{
    virt_viewer_window_menu_view_set_resolution(NULL, VIRT_VIEWER_WINDOW(user_data));
}
...
static const GActionEntry keypad_action_entries[] = {
    ...
    { .name = "set-resolution", .activate = action_set_resolution },
};
...
static void toolbar_preference_location(GtkMenu *menu G_GNUC_UNUSED, gint *x, gint *y,
                                   gboolean *push_in, gpointer user_data)
{
    VirtViewerWindow *self = user_data;
    GtkAllocation allocation;
    GtkWidget *toplevel = gtk_widget_get_toplevel(self->priv->toolbar_preference);

    *push_in = TRUE;
    gdk_window_get_origin(gtk_widget_get_window(toplevel), x, y);
    gtk_widget_translate_coordinates(self->priv->toolbar_preference, toplevel,
                                     *x, *y, x, y);
    gtk_widget_get_allocation(self->priv->toolbar_preference, &allocation);
    *y += allocation.height;
}

static void
virt_viewer_window_toolbar_preference(GtkWidget *button G_GNUC_UNUSED,
                                    VirtViewerWindow *self)
{
    GtkMenu *menu = virt_viewer_window_get_preference_menu(self);
    gtk_menu_attach_to_widget(menu, self->priv->window, NULL);
    g_object_ref_sink(menu);
    gtk_menu_popup(menu, NULL, NULL, toolbar_preference_location, self,
                   0, gtk_get_current_event_time());
    g_object_unref(menu);
}


static void toolbar_file_location(GtkMenu *menu G_GNUC_UNUSED, gint *x, gint *y,
                                        gboolean *push_in, gpointer user_data)
{
    VirtViewerWindow *self = user_data;
    GtkAllocation allocation;
    GtkWidget *toplevel = gtk_widget_get_toplevel(self->priv->toolbar_file);

    *push_in = TRUE;
    gdk_window_get_origin(gtk_widget_get_window(toplevel), x, y);
    gtk_widget_translate_coordinates(self->priv->toolbar_file, toplevel,
                                     *x, *y, x, y);
    gtk_widget_get_allocation(self->priv->toolbar_file, &allocation);
    *y += allocation.height;
}

static void
virt_viewer_window_toolbar_file(GtkWidget *button G_GNUC_UNUSED,
                                      VirtViewerWindow *self)
{
    GtkMenu *menu = virt_viewer_window_get_file_menu(self);
    gtk_menu_attach_to_widget(menu, self->priv->window, NULL);
    g_object_ref_sink(menu);
    gtk_menu_popup(menu, NULL, NULL, toolbar_file_location, self,
                   0, gtk_get_current_event_time());
    g_object_unref(menu);
}
                                   ...
                                   VirtViewerWindow *self)
{
    ...
    icon = GTK_WIDGET(gtk_builder_get_object(about, "icon"));
    buildtime = GTK_WIDGET(gtk_builder_get_object(about, "buildtime"));
    close = GTK_WIDGET(gtk_builder_get_object(about, "close"));
    uri = GTK_WIDGET(gtk_builder_get_object(about, "uri"));

    pixbuf = gdk_pixbuf_new_from_resource(VIRT_VIEWER_RESOURCE_PREFIX"/icons/48x48/virt-viewer.png", NULL);

    gtk_image_set_from_pixbuf(GTK_IMAGE(icon), pixbuf);
    
    gtk_label_set_markup(GTK_LABEL(uri),"<a href=\"http://www.topsec.com.cn\"> www.topsec.com.cn </a>");
...
virt_viewer_window_toolbar_setup(VirtViewerWindow *self)
{
    ...
    /* Close connection */
    icon_widget = gtk_image_new_from_resource(VIRT_VIEWER_RESOURCE_PREFIX"/icons/48x48/disconnect.png");
    button = GTK_WIDGET(gtk_tool_button_new(icon_widget, NULL));
    gtk_tool_button_set_label(GTK_TOOL_BUTTON(button), _("Disconnect"));
    ...
    */
    ...
    /* Leave fullscreen */
    icon_widget = gtk_image_new_from_resource(VIRT_VIEWER_RESOURCE_PREFIX"/icons/48x48/leave-fullscreen.png");
    button = GTK_WIDGET(gtk_tool_button_new(icon_widget, NULL));
    ...
    /* preference */
    /*
    icon_widget = gtk_image_new_from_resource(VIRT_VIEWER_RESOURCE_PREFIX"/icons/48x48/preference.png");
    button = GTK_WIDGET(gtk_tool_button_new(icon_widget, NULL));
    gtk_tool_button_set_label(GTK_TOOL_BUTTON(button), _("preference"));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("preference"));
    gtk_tool_item_set_is_important(GTK_TOOL_ITEM(button), TRUE);
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM(button), 0);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_toolbar_preference), self);
    gtk_widget_set_sensitive(button, TRUE);
    priv->toolbar_preference = button;
    gtk_widget_show_all(button); 
    */ 

    /* Send key */
    icon_widget = gtk_image_new_from_resource(VIRT_VIEWER_RESOURCE_PREFIX"/icons/48x48/send-key.png");
    button = GTK_WIDGET(gtk_tool_button_new(icon_widget, NULL));
    gtk_tool_button_set_label(GTK_TOOL_BUTTON(button), _("Send key combination"));
    ...
    /* file */
    icon_widget = gtk_image_new_from_resource(VIRT_VIEWER_RESOURCE_PREFIX"/icons/48x48/file.png");
    button = GTK_WIDGET(gtk_tool_button_new(icon_widget, NULL));
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_menu_file_usb_device_selection), self);
    gtk_widget_set_sensitive(button, TRUE);
    priv->toolbar_file = button;
    gtk_widget_show_all(button);
    
    /* home */
    /*
    icon_widget = gtk_image_new_from_resource(VIRT_VIEWER_RESOURCE_PREFIX"/icons/48x48/home.png");
    button = GTK_WIDGET(gtk_tool_button_new(icon_widget, NULL));
    gtk_tool_button_set_label(GTK_TOOL_BUTTON(button), _("home"));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("home"));
    gtk_tool_item_set_is_important(GTK_TOOL_ITEM(button), TRUE);
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM(button), 0);
    //g_signal_connect(button, "clicked", G_CALLBACK(), self);
    gtk_widget_show_all(button); 
    */ 
...
if (!ungrab && !priv->subtitle){//获取写死的后半部分标题字�?
    ...
    /* translators:
         ...
         */
     char * pstr= priv->subtitle;
    if (NULL != g_strrstr(pstr, ":"))
    {
      pstr = "";
      title = g_strdup_printf(_("%s%s%s%s"),//获取写死的后半部分标题字�?用户自定义写入的前半部分标题字段
...
virt_viewer_window_set_usb_options_sensitive(VirtViewerWindow *self, gboolean sensitive)
{
    ...
    gtk_widget_set_sensitive(menu, sensitive);
    gtk_widget_set_visible(priv->toolbar_file, sensitive);
}
...
void virt_viewer_window_change_watermark_to_static(guint interval, gfloat x, gfloat y)
{
    if (1 == interval) {
        watermark_status = 0;
	watermark_x = x;
...
//新增定时器随机更新水印label的显示位�?
static gboolean virt_viewer_overlay_label_random_moving(gpointer data)
{
    VirtViewerWindowPrivate *win = data;
	gfloat xalign, yalign;
 
	xalign = g_random_double_range (0.0, 1.0);
	yalign = g_random_double_range (0.0, 1.0);

	gtk_label_set_xalign(GTK_LABEL(win->label), xalign);
	gtk_label_set_yalign(GTK_LABEL(win->label), yalign); 
	
        if (watermark_status){
            return G_SOURCE_CONTINUE;
        }else{
		gtk_label_set_xalign(GTK_LABEL(win->label), watermark_x);
		gtk_label_set_yalign(GTK_LABEL(win->label), watermark_y);
		return G_SOURCE_REMOVE;
	}
}

//�����Զ���ˮӡλ��
static gboolean virt_viewer_overlay_label_random_moving_x_y(gpointer data, gdouble watermark_x, gdouble watermark_y)
{
    VirtViewerWindowPrivate* win = data;
...
void virt_viewer_window_change_watermark_to_dynamic(VirtViewerWindow *self, guint interval)
{
    VirtViewerWindowPrivate *priv = self->priv;
    if (0 != interval) {
        watermark_status = 1;
        g_timeout_add_seconds (interval, virt_viewer_overlay_label_random_moving, priv);
    }
}

static void file_transfer_cb(GObject *source_object,
                                   GAsyncResult *result,
                                   gpointer user_data G_GNUC_UNUSED)
{
...
get_main_channel(VirtViewerDisplay *self)
{
...
void drag_data_received_cb(VirtViewerDisplay *self G_GNUC_UNUSED,
                                        ...
                                        gpointer *user_data)
{
    g_print("drag_data_received_callback entry... \n");
    const guchar *buf;
    gchar **file_urls;
    int n_files;
    int i = 0;
    GFile **files;
	VirtViewerDisplay *display = user_data;
    ...
    spice_main_channel_file_copy_async(get_main_channel(display), files, 0, NULL, NULL, NULL, file_transfer_cb,
                                       NULL);
    for (i = 0; i < n_files; i++) {
...
virt_viewer_window_set_display(VirtViewerWindow *self, VirtViewerDisplay *display)
{
    ...
    if (display != NULL) {
        ...
        //水印信息通过参数传进来，格式：内容[,水印移动时间,字体,大小,粗细,颜色]
        gchar *content;
        gint interval     = 4;
	gchar *slant_style = "normal";
	gint font_size    = 9;
	gint font_weight  = 20;
	gchar *font_color = "black";
        gchar *font_style = "normal";
        gchar *underline_type = "none";
        gchar *back_color = "white";
        gdouble alpha_percent = 1;
        guint alpha = 65535;//背景颜色透明�?0,65536)
		//新增自定义静止水印x,y位置坐标 2021.01.22 xc
		gdouble watermark_x	= 0.0;//水印自定义位�?横坐�?
		gdouble watermark_y	= 0.0;//水印自定义位�?纵坐�?
        if (priv->watermark_info != NULL) {
            g_print("%s\n",priv->watermark_info);
            gchar **infos = g_strsplit(priv->watermark_info, ",", 12);//改之前默�?0个参�?
	    guint infos_cnt = g_strv_length(infos);

	     for(int i=1; i<=infos_cnt; i++)//后续借用switch优化
    {
      switch(i)
      {
        case 1: content = infos[0];break;
        case 2:{
          interval = strtol(infos[1], NULL, 10);
          if (interval < 0) {
            interval = 4;
          }break;} 
        case 3:{
          slant_style = infos[2];break;}
        case 4:{
          font_size = strtol(infos[3], NULL, 10);    
          if (font_size <= 0){
            font_size = 9;}break;}
        case 5:{
          font_weight = strtol(infos[4], NULL, 10);
          if (font_weight <= 0) {
            font_weight = 20;
          }break;}
        case 6:{
        ...
        case 7:{
          ...
          break;}
        case 8:{
          underline_type = infos[7];
          break;}
        case 9:{
          back_color = infos[8];
          break;}
        case 10:{
          alpha_percent = g_ascii_strtod(infos[9], NULL);
          if (1.0 == alpha_percent) {
            alpha = 1;
          }else{
            alpha *= (1.0-alpha_percent);}break;
        }
		case 11:{			
          watermark_x = g_ascii_strtod(infos[10], NULL);//x
		  printf("start: watermark_x=%.2f\n",watermark_x);
		  if(watermark_x > 1.0)
		...
		case 12:{
		  ...
		  printf("start: watermark_y=%.2f\n",watermark_y);
		  if(watermark_y > 1.0)
			watermark_y = g_random_double_range(0.0, 1.0);
		  break;
        }
        default : printf("ERROR:infos_cnt = %d\n", infos_cnt);break;
      }    
    }   
  ...
  //优化markup组合字段参数，应使用分参数获�?2020.09.25 xc add 取消字体倾斜（因为window部分中文字体显示有乱码，暂时屏蔽处理�?
    const char *format = "<span font_family='%s' style='%s' foreground='%s' weight='%d' font='%d' background='%s' background_alpha='%u' underline='%s'>%s</span>";//栈上释放
    //const char *format = "<span font_family='%s' foreground='%s' weight='%d' font='%d' background='%s' background_alpha='%u' underline='%s'>%s</span>";//栈上释放
    char *markup;
    markup = g_markup_printf_escaped (format, font_style,slant_style,font_color, font_weight, font_size, back_color, alpha, underline_type, content);
    //font_style //slant_style font_color font_weight font_size back_color alpha_percent underline_type content
    gtk_label_set_markup (GTK_LABEL (priv->label), markup);
    g_free (markup);
		
        gtk_overlay_add_overlay (GTK_OVERLAY (priv->overlay), GTK_WIDGET(display));
        gtk_overlay_set_overlay_pass_through (GTK_OVERLAY (priv->overlay), GTK_WIDGET(display), FALSE);

        gtk_overlay_add_overlay (GTK_OVERLAY (priv->overlay), priv->label); 
        gtk_overlay_set_overlay_pass_through (GTK_OVERLAY (priv->overlay), priv->label, TRUE); 
        

        if (0 != interval) {//0代表静止水印
            gtk_label_set_xalign(GTK_LABEL(priv->label), g_random_double_range(0.0, 1.0));
            gtk_label_set_yalign(GTK_LABEL(priv->label), g_random_double_range (0.0, 1.0)); 
        }else{
			gtk_label_set_xalign(GTK_LABEL(priv->label), watermark_x);
            gtk_label_set_yalign(GTK_LABEL(priv->label), watermark_y);
		}
        ...
        gtk_notebook_append_page(GTK_NOTEBOOK(priv->notebook), GTK_WIDGET(priv->overlay), NULL);
        //gtk_notebook_append_page(GTK_NOTEBOOK(priv->notebook), GTK_WIDGET(display), NULL);

        gtk_widget_realize(GTK_WIDGET(priv->overlay));
        //gtk_widget_realize(GTK_WIDGET(display));

        if (priv->overlay_lock == FALSE && 0 != interval) {
            g_print("g_timeout_add_seconds\n");
            g_timeout_add_seconds (interval, virt_viewer_overlay_label_random_moving, priv);
            priv->overlay_lock = TRUE; //避免重复添加定时�? 
        }

	GtkTargetEntry targets = { "text/uri-list", 0, 0};
        gtk_drag_dest_set(GTK_WIDGET(priv->window), GTK_DEST_DEFAULT_ALL, &targets, 1, GDK_ACTION_COPY);
	virt_viewer_signal_connect_object(priv->window, "drag-data-received",
						 G_CALLBACK(drag_data_received_cb), display, 0); 
