...
static BOOL wf_pre_connect(freerdp* instance)
{
	...
	wfc->fullscreen = settings->Fullscreen;//全屏
	//wfc->fullscreen = 1;
...
static WCHAR* wf_window_get_title(rdpSettings* settings)
{
	...
	size_t size;
	char* name;
	WCHAR prefix[] = L"FreeRDP:";//远程桌面窗口标题前缀
...
//static CREDUI_INFOA wfUiInfo = { sizeof(CREDUI_INFOA), NULL, "Enter your credentials",
	                             //"Remote Desktop Security@Xtopsec", NULL };

static CREDUI_INFOA wfUiInfo = { sizeof(CREDUI_INFOA), NULL, "Please Input Your Data",
