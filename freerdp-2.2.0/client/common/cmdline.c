...
static void freerdp_client_print_command_line_args(COMMAND_LINE_ARGUMENT_A* arg)
{
	...
	do
	{
		...
		//临时打印记录
		char* RdpStr;
		RdpStr = (char *) malloc(sizeof(char)*128);
		sprintf(RdpStr, "%s%s", arg->Default ? "-" : "+", arg->Name);
		...
		//#COMMAND_LINE_VALUE_REQUIRED 0x00000002	COMMAND_LINE_VALUE_OPTIONAL 0x00000004
		...
		//COMMAND_LINE_VALUE_BOOL 0x00000008
		if (arg->Flags & COMMAND_LINE_VALUE_BOOL)
		{
                                               ...
                                               COMMAND_LINE_ARGUMENT_A* custom)
{
	...
	char* szUsername;
	int WRdpRet;
	szUsername = (char*)malloc(sizeof(char) * 2056);	
	WRdpRet = 0;
	...
	sprintf(szUsername,
	        ...
	        "option with value\n  +toggle -toggle enables or disables toggle,where '/' is a "
	        "synonym of '+'\n\n", argv[0]);	
	//WRdpRet =
	    //MessageBoxA(0, (LPCSTR)szUsername, (LPCSTR)("帮助信息提示"), MB_YESNO | MB_ICONQUESTION);
	...
	sprintf(szUsername,
	        ...
	        "/parallel:<name>,<device>\n Printer Redirection: /printer:<device>,<driver>\n TCP "
	        "redirection: /rdp2tcp:/usr/bin/rdp2tcp\n \n Audio Output Redirection: "
	        "/sound:sys:oss,dev:1,format:1\n Audio Output Redirection: /sound:sys:alsa\n Audio "
	        "Input Redirection: /microphone:sys:oss,dev:1,format:1\n Audio Input Redirection: "
	        "/microphone:sys:alsa\n \n Multimedia Redirection: /video\n",name, name, name, name);
	//WRdpRet =
	    //MessageBoxA(0, (LPCSTR)szUsername, (LPCSTR)("帮助信息提示"), MB_YESNO | MB_ICONQUESTION);
#ifdef CHANNEL_URBDRC_CLIENT
	//显示帮助提示消息弹窗
	sprintf(szUsername, "USB Device Redirection: /usb:id:054c:0268#4669:6e6b,addr:04:0c\n");
	//WRdpRet =
	    //MessageBoxA(0, (LPCSTR)szUsername, (LPCSTR)("帮助信息提示"), MB_YESNO | MB_ICONQUESTION);
...
#ifdef _WIN32
	//显示帮助提示消息弹窗
	//sprintf(szUsername, "    set HTTPS_PROXY=http://proxy.contoso.com:3128/\n");
	//WRdpRet =
	    //MessageBoxA(0, (LPCSTR)szUsername, (LPCSTR)("帮助信息提示"), MB_YESNO | MB_ICONQUESTION);
...
#else
	//显示帮助提示消息弹窗
	//sprintf(szUsername, "    export https_proxy=http://proxy.contoso.com:3128/\n");
	//WRdpRet =
	   // MessageBoxA(0, (LPCSTR)szUsername, (LPCSTR)("帮助信息提示"), MB_YESNO | MB_ICONQUESTION);
	...
	//显示帮助提示消息弹窗
	//sprintf(szUsername,
	        //"    %s /g:rdp.contoso.com ...\n\nMore documentation is coming, in the meantime "
                                                            ...
                                                            BOOL ignoreUnknown)
{
	...
	*count = 0;
	...
	arg = largs;
	//遍历164组功能参数，计数功能参数<arg->Flags == COMMAND_LINE_ARGUMENT_PRESENT>（0x80000000）
	do
	{
                                                         ...
                                                         COMMAND_LINE_ARGUMENT_A* custom)
{
	...
	if (status == COMMAND_LINE_STATUS_PRINT_VERSION)//COMMAND_LINE_STATUS_PRINT_VERSION -2003
	{
		freerdp_client_print_version();
		return COMMAND_LINE_STATUS_PRINT_VERSION;
	}

	if (status == COMMAND_LINE_STATUS_PRINT_BUILDCONFIG)//COMMAND_LINE_STATUS_PRINT_BUILDCONFIG -2004
	{
		freerdp_client_print_version();
		freerdp_client_print_buildconfig();
		return COMMAND_LINE_STATUS_PRINT_BUILDCONFIG;
	}
	else if (status == COMMAND_LINE_STATUS_PRINT)//COMMAND_LINE_STATUS_PRINT -2001
	{
		COMMAND_LINE_ARGUMENT_A largs[ARRAYSIZE(args)];
		...
		if (arg->Flags & COMMAND_LINE_ARGUMENT_PRESENT)//COMMAND_LINE_ARGUMENT_PRESENT 0x80000000
		{
		...
		if (arg->Flags & COMMAND_LINE_VALUE_PRESENT)//COMMAND_LINE_VALUE_PRESENT 0x40000000
		{
			DWORD i;
			RDP_KEYBOARD_LAYOUT* layouts;
			layouts = freerdp_keyboard_get_layouts(RDP_KEYBOARD_LAYOUT_TYPE_STANDARD);//RDP_KEYBOARD_LAYOUT_TYPE_STANDARD 1
			// if (!layouts) /* FIXME*/
			printf("\nKeyboard Layouts\n");
			...
			freerdp_keyboard_layouts_free(layouts);
			layouts = freerdp_keyboard_get_layouts(RDP_KEYBOARD_LAYOUT_TYPE_VARIANT);//RDP_KEYBOARD_LAYOUT_TYPE_VARIANT 2
			// if (!layouts) /* FIXME*/
			...
			freerdp_keyboard_layouts_free(layouts);
			layouts = freerdp_keyboard_get_layouts(RDP_KEYBOARD_LAYOUT_TYPE_IME);//RDP_KEYBOARD_LAYOUT_TYPE_IME 4
			// if (!layouts) /* FIXME*/
	...
	else if (status < 0)
	{
		...
		return COMMAND_LINE_STATUS_PRINT_HELP;//COMMAND_LINE_STATUS_PRINT_HELP -2002
	}
                                                         ...
                                                         char** argv, BOOL allowUnknown)
{
	...
	do
	{
		BOOL enable = arg->Value ? TRUE : FALSE;//判断已经在命令行输入的功能参数的具体数值（设置）
