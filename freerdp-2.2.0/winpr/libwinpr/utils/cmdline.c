	...
	for (i = 1; i < argc; i++)
	{
		    ...
		    (flags & COMMAND_LINE_SIGIL_NOT_ESCAPED))
		{
			...
			if ((flags & COMMAND_LINE_SEPARATOR_COLON) && (!separator))
				separator = strchr(keyword, ':');//:Xtopsec
			...
			//对options组内164个数据组进行判断并匹配具体参数
			for (j = 0; options[j].Name != NULL; j++)
			{
				char* Rkeyword = options[j].Name;
				BOOL match = FALSE;
				...
				if ((flags & COMMAND_LINE_SEPARATOR_SPACE) && ((i + 1) < argc))
				{
					...
					unsigned long RFlags = options[j].Flags;
