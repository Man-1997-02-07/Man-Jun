...
rdpSettings* freerdp_settings_new(DWORD flags)
{
	...
	settings->DesktopHeight = 900;//(生效！)
	settings->Workarea = FALSE;
	//settings->Workarea = TRUE;
	settings->Fullscreen = FALSE;//全屏关闭(生效！)
	//settings->Fullscreen = TRUE;//全屏打开
	//settings->GrabKeyboard = TRUE;//抢键盘默认打开，影响快捷键
	...
	//settings->KeyboardHook = KEYBOARD_HOOK_LOCAL;
	/*
	    KEYBOARD HOOK LOCAL 键盘挂钩本地
	    KEYBOARD HOOK REMOTE 键盘挂钩远程
	    KEYBOARD HOOK FULLSCREEN ONLY 键盘挂钩全屏显示
	*/
	settings->UseRdpSecurityLayer = FALSE;
	settings->SaltedChecksum = TRUE;
	settings->ServerPort = 3389;//(生效！)
	settings->GatewayPort = 443;//(生效！)
	settings->DesktopResize = TRUE;
	//settings->ToggleFullscreen = FALSE; //全屏切换打开ctrl+alt+enter
	...
	settings->DisableFullWindowDrag = TRUE;//禁用全窗口拖动(未生效！)
	...
	// settings->AutoReconnectionEnabled = FALSE;//自动重新连接已启用
	...
	if (!settings->ServerMode)
	{
		//settings->RedirectClipboard = TRUE;//默认打开剪切板重定向
